using System.Reflection;
using System.Runtime.CompilerServices;
using System.Runtime.InteropServices;

#if DEBUG
[assembly: AssemblyTitle("SenseNet.Notification.Portlets (Debug)")]
#else
[assembly: AssemblyTitle("SenseNet.Notification.Portlets (Release)")]
#endif

[assembly: AssemblyDescription("Notification Portlets for the sensenet ECM platform.")]
[assembly: AssemblyConfiguration("")]
[assembly: AssemblyCompany("Sense/Net Inc.")]
[assembly: AssemblyCopyright("Copyright © Sense/Net Inc.")]
[assembly: AssemblyProduct("sensenet ECM")]
[assembly: AssemblyTrademark("Sense/Net Inc.")]
[assembly: AssemblyCulture("")]
[assembly: AssemblyVersion("7.1.0.0")]
[assembly: AssemblyFileVersion("7.1.0.0")]
[assembly: AssemblyInformationalVersion("7.1.0")]

[assembly: ComVisible(false)]
[assembly: Guid("c62de588-dd3d-495e-8288-ece36ed42a60")]
